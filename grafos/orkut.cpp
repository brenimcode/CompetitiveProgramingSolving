...
vector<string> adj[100005]; // Vetor de adjacências para representar as relações de dependência
int grauEntrada[100005]; // Vetor para contar o grau de entrada de cada tarefa
vector<string> ordem; // Vetor para armazenar a ordem ótima de execução das tarefas
...
int main() {
    ...
    while(cin >> n){ // Leitura do número de tarefas e de relações de dependência
    if(!n) break;
    ...
    if (topologicalSort(n)) {
        ...
        cout << "impossivel" << endl; // Caso não seja possível ordenar as tarefas
    }
    }
    return 0;
}
